...
<template lang="md">
# Скачивание ассетов/клиентов Minecraft с официального зеркала Mojang {.head}

<br>

Почти всё, что будет описано выше, является вольным пересказом <a href="https://wiki.vg/Game_files" target="_blank">следующей статьи</a>, а также некоторой информацией взятой из личного опыта.

Все доступные версии самой игры перечислены в файле <a href="https://launchermeta.mojang.com/mc/game/version_manifest.json" target="_blank">launchermeta.mojang.com/mc/game/version_manifest.json</a>
...
Далее в тексте для примера мы будем разбирать работу оригинального лаунчера Minecraft и <a href="https://launchermeta.mojang.com/v1/packages/436877ffaef948954053e1a78a366b8b7c204a91/1.16.5.json" target="_blank">JSON файл для версии 1.16.5</a>
...
*Файлы "нативок" хоть и в формате jar, но не содержат скомпилированный java код, а являются, по сути, просто архивами с нативными файлами (dll, so, dylib, jnilib).*
...
## Sashok724 v3 / KeeperJerry Launcher (sashok724 v3 fork) / GravitLauncher
...
Для скачивания ассетов и клиентов можно использовать встроенные команды `downloadassets` и `downloadclient` с указанием типа `mojang`.

## Nexte Launcher

Ассеты в данном лаунчере хранятся в общей папке, клиенты по отдельности.  
Структура папки клиента аналогична структуре других лаунчеров.

Также можно скачивать ассеты и клиенты встроенными командами `downloadassets` и `downloadclient` с указанием флага `-m` или `--mojang`.

## NSLauncher

В данном лаунчере используется своя структура хранения файлов, подробнее в <a href="https://launcher.team-ns.net/0.0.1/profiles.html#структура-профиля" target="_blank">документации лаунчера</a>.

Также для загрузки ассетов и клиентов можно использовать утилиту <a href="https://github.com/team-ns/profile-generator" target="_blank">profile-generator</a>, подробнее в <a href="https://launcher.team-ns.net/0.0.1/profiles.html#генерация-профилеи" target="_blank">документации</a>.
</template>
